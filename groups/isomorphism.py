...
class Isomorphism:
	...
	def group_different(G: Group, H: Group) -> bool:
		...
		# This is a quick check to filter out groups that are non-isomorphic
		if G.order != H.order:
			return True
	...
	@staticmethod
	def are_isomorphic(G: Group, H: Group, method="gap"):
		# Remove easy cases before testing
	...
	def isomorphism_brute(G: Group, H: Group) -> bool:
		...
		for codomain_candidate in (
			tqdm(codomain_iterator, total=iterator_len)
			if VERBOSE
			else codomain_iterator
		):
			codomain = sum(map(list, codomain_candidate), [])
			mapping = dict(zip(domain, codomain))
			if check_isomorphism(G, H, lambda g: mapping[g]):
				if VERBOSE:
					print(mapping)
				return True
		return False
